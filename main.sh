...
sudo pacman --noconfirm -Sy archlinux-keyring parabola-keyring
...
if [ $? -ne 0 ]; then
	nmtui || (pacman -S networkmanager && nmtui)
fi
...
if [[ "$bcfg" == "bios" ]]; then
    pacstrap /mnt base base-devel linux-libre wpa_supplicant libelogind udev-init-scripts networkmanager parabola-base
elif [[ "$bcfg" == "uefi" ]]; then
    pacstrap /mnt base base-devel linux-libre efibootmgr wpa_supplicant libelogind udev-init-scripts networkmanager parabola-base
fi
...
if [[ "$desktop_env" == "xfce" ]]; then
    ...
    pacman --noconfirm -S xfce4 xfce4-goodies openrc-desktop elogind
elif [[ "$desktop_env" == "gnome" ]]; then
    echo "Installing GNOME..."
    pacman --noconfirm -S gnome openrc-desktop elogind
elif [[ "$desktop_env" == "kde" ]]; then
    echo "Installing KDE..."
    pacman --noconfirm -S plasma kde-applications openrc-desktop elogind
elif [[ "$desktop_env" == "none" ]]; then
    echo "Installing NetworkManager..."
    pacman --noconfirm -S networkmanager openrc-desktop elogind
